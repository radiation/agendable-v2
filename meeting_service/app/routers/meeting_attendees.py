...
from schemas import meeting_attendee_schemas as schemas
...
def read_attendees_by_meeting(meeting_id: int, db: Session = Depends(db.get_db)):
...
def read_meetings_by_user(user_id: int, db: Session = Depends(db.get_db)):
